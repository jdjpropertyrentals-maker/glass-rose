...
async function main(){
  ...
  if (res.rows.length === 0) {
    console.log('No pending deposits to reconcile')
    await client.end()
    return
  }
  
  console.log(`Reconciling ${res.rows.length} pending deposits...`)
  
  // Process all deposits in a single transaction for better performance
  try {
    ...
    for (const row of res.rows){
      ...
      try {
        ...
        throw err // Rollback entire batch on any error
      }
    }
    
    await client.query('COMMIT')
    console.log(`Successfully reconciled ${res.rows.length} deposits`)
  } catch (err){
    await client.query('ROLLBACK')
    console.error('Batch reconciliation failed, rolled back all changes', err)
  }
